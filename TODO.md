...
Constructor call refactor: DONE. `check_constructor_call` removed; the two callsites in `check_generic_call` route through a shared `check_pure_struct_construction` helper that also backs `check_call`'s pure-struct branch. `Foo(_, x)` underscore substitution works against field defaults. Constructor body codegen rewritten — fields pre-bind to GEPs into %sret so writes go straight to the caller's slot; the per-field alloca + copy-locals-to-sret epilogue is gone (-58 allocas in test.ll). `check_dispatch_call` now also handles `_` — each underscore matches any candidate that has a default at that position, and the picked overload's defaults substitute in before resolution. `check_dispatch_call` still doesn't do `fill_default_args` (call arity must match a candidate exactly) — separate work if dispatch needs to grow trailing-default support.
