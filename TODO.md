...
DECIDE assert policy: keep asserts on in release (TigerStyle, restart-and-recover fits the hot-reload loop) or C-style strip. (Output upgrade DONE 10 Jun 2026: "Assert failed at <loc> / Expected game.running == false, but game.running was true" — comparison asserts name each non-literal operand with its value, bools print true/false, condition text keeps source spacing. Enum/char operands still print numerically; upgrade via gen_print_enum machinery if wanted.)
