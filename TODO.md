...
Constructor call refactor: DONE for the args→fields/params split. `check_constructor_call` removed; the two callsites in `check_generic_call` route through a shared `check_pure_struct_construction` helper, which is also called from `check_call`'s pure-struct branch. `Foo(_, x)` underscore substitution now works against field defaults. Constructor body codegen also rewritten — fields pre-bind to GEPs into %sret so writes go straight to the caller's slot; the per-field alloca + copy-locals-to-sret epilogue is gone (-58 allocas in test.ll). Still TODO: `check_dispatch_call` doesn't run underscore substitution or default fill — `mul(_, x)` over a dispatch group won't substitute. Same shared engine should serve it.
