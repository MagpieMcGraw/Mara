...
slice_seed :: proc(s: ^Slice, stmts: []Stmt) {
    for st in stmts {
        ...
        case Stmt_Return: for e in v.values { slice_value(s, e) }
...
@(private="file")
slice_run :: proc(checked: ^Checked_Program, ft: ^Type_Scope) -> [dynamic]^Def {
    s := Slice{ checked = checked }
    defer { delete(s.seen_use); delete(s.seen_def); delete(s.work) }
    slice_seed(&s, ft.body[:])
    slice_seed_named_returns(&s, ft)
    for len(s.work) > 0 {
        u := pop(&s.work)
        for d in checked.reaching[u] {
            if s.seen_def[d] { continue }
            s.seen_def[d] = true
            append(&s.result, d)
            slice_value(&s, d.value)   // a param has nil value — a slice boundary
        }
    }
    return s.result
}
...
render_contributors :: proc(checked: ^Checked_Program, ft: ^Type_Scope, label: string) -> string {
    ...
    fmt.sbprintf(&b, "%s — contributors to the return value  (backward data slice)\n", label)
    if len(params) > 0 {
        fmt.sbprintf(&b, "\n  parameters (%d)\n", len(params))
    ...
    fmt.sbprint(&b, "\n  note: data dependencies only — the branch conditions that guard these\n")
    fmt.sbprint(&b, "        statements (control dependence) are not included yet.\n")
